...
def create_swa_mla_moe_config(args):
    """Create SWA-MLA-MOE (Sliding Window Attention + Multi-head Latent Attention + Mixture of Experts) configuration.

    SWA-MLA-MOE combines efficient sliding window attention with MLA's latent compression and MoE for scaling.

    Args:
        args: Training arguments (same as create_swa_mla_config plus):
            - num_experts: Number of experts (default: 8)
            - experts_per_token: Experts selected per token (default: 2)

    Returns:
        SWAMLAMOEConfig: Configuration for SWA-MLA-MOE model
    """
    from models.models.swa_mla_moe_model import SWAMLAMOEConfig
